...
import { generateWhatsAppCheckoutURL } from "@/lib/utils/whatsapp";
...
export function CartSummary() {
  ...
  return (
    <div className="rounded-2xl backdrop-blur-xl bg-white/5 border border-white/10 p-6 space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-sm">
          ...
          <span className="text-white/80">
            {formatCurrency(totalPrice, config.currencySymbol)}
          </span>
        </div>
      </div>
      ...
      <button
        onClick={clearCart}
        className="w-full text-sm text-white/40 hover:text-red-400 transition-colors"
      >
        Clear Cart
      </button>
    </div>
